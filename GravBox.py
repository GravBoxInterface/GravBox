...
class Particle(Widget):
    ...
    #def __init__(self, pos, size):
        ...
        with self.canvas:
            Color(1, 1, 1)
            d = 10
            self.ellipse = Ellipse(pos=pos, size=(d,d))
        
        self.bind(pos = self.updateParticle)
            #d = 30
            #self.particleSprite = Ellipse(pos=(pos[0] - d/2, pos[1] - d/2), size=(d, d)) #source=someimage, pos, size
            #self.particleSprite = Ellipse(pos = self.pos, size = (d,d))
            #self.bind(pos=self.updateParticle)
            #self.x = self.center_x
            #self.y = self.center_y
            #self.pos = (self.x - d/2, self.y - d/2)
            #self.particleSprite.pos = self.pos
                        
            
    #REMOVING/UPDATING PARTICLE TO FIX ANIMATION    
    #def updateParticle(self, instance, value):
        #self.canvas.clear()
        #self.ellipse.pos = self.pos
        #self.particleSprite.pos = value
...
class InteractionScreen(Screen): #This is the main screen for drawing and user input. Next steps - adding/binding buttons
    ...
    def userInput(self, *args): #This function to be used when the go button is relased, this will need input data from DrawingApp class
        ...
        with open(inputFileName, 'w') as f:
	        f.write('%f\t%f\t%f\t%f\t' % (drawUtility.x_initial, drawUtility.y_initial, drawUtility.x_final, drawUtility.y_final))#, drawUtility.x_delta, drawUtility.y_delta))
        ...
        '''
        p = pathlib.Path(assetsdirectory+str('algorithm_output1.txt'))
        ...
        while os.path.isfile('algorithm_output.csv') == False:
            ...
            print ('its here')
            #x, y = np.loadtxt('algorithm_output.csv', delimiter=',', usecols=(0,1), unpack=True)
            x, y = np.loadtxt(assetsdirectory+str('algorithm_output.csv'), usecols=(0,1), unpack=True)
        ...
        def algorithmOutput(self, *args):
            pass
            
            
        if p.is_file():
            x, y = np.loadtxt(assetsdirectory+str('algorithm_output1.txt'), usecols=(0,1), unpack=True)
            animation = Animation(pos=(int(round(x[0])), int(round(y[0]))), t='linear', duration=.05)
            for i in np.arange(0, 198):
                print(i, int(round(x[i])), int(round(y[i])))
                animation += Animation(pos=(int(round(x[i])), int(round(y[i]))), t='linear', duration=.05)
            animation.start(particle)
        '''
        ...
        while os.path.isfile('algorithm_output.npy') == False:
            ...
            #x,y = np.load('mylistx.npy');
            
            try:
                xs = np.split(x, len(x)/200, 0); 
                ys = np.split(y, len(y)/200, 0)
            except:
                xs = np.split(x, len(x)/2, 0);
                ys = np.split(y, len(y)/2, 0)
    ...
    def animate(self, x_coords, y_coords, index, instance):
        ...
        if index < len(x_coords):
            ...
            animation = Animation(pos=(int(round(x_coords[index][0])), int(round(y_coords[index][0]))), t='linear', duration=.02)
        
            for i in np.arange(0, len(x_coords[index])):
                animation += Animation(pos=(int(round(x_coords[index][i])), int(round(y_coords[index][i]))), t='linear', duration=.02)
