...
class DataImport(Widget): #NOT SURE IF THIS IS FEASIBLE FOR DATA INPUT
    
    def __init__(self, **kwargs):
        super(DataImport, self).__init__(**kwargs)
        
    def fileToList(self, instance, value):
        
        with open('algorithm_output.txt') as f:
            animList = f.readlines()
            print (animList)
