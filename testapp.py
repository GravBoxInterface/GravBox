...
class DrawingApp(Widget):
    ...
    def on_touch_down(self, touch):
        with self.canvas:
            ...
            if touch.x > 900:
                ...
                GlobalVarFileChange('global.txt', 0, str(self.x_initial))
                GlobalVarFileChange('global.txt', 1, str(self.y_initial))
    ...
    def on_touch_move(self, touch):
        with self.canvas:
            ...
            if touch.x <= 900 and self.out_of_bounds == False:
                d = 10
                Ellipse(pos=(touch.x - d/2, touch.y - d/2), size=(d,d))
                touch.ud['line'] = Line(points=(touch.x, touch.y))
            ...
            elif touch.x > 900 and self.out_of_bounds == False:
                d = 10
                Ellipse(pos=(900 - d/2, touch.y - d/2), size = (d,d))
                touch.ud['line'] = Line(points=(touch.x, touch.y))
                self.x_final = 900
                self.y_final = touch.y
                print (self.x_final)
                print (self.y_final)
                Line(points=[self.x_initial, self.y_initial, 900, touch.y])
                
            elif touch.x > 900 and self.out_of_bounds == True:
    ...
    def on_touch_up(self, touch):
        with self.canvas:
            ...
            if touch.x > 900 and self.out_of_bounds == False:
                d = 10
                Ellipse(pos=(900 - d/2, touch.y - d/2), size=(d,d))
                self.x_delta = (900 - self.x_initial)
                self.y_delta = (touch.y - self.y_initial)
                print (self.x_delta)
                print (self.y_delta)
                GlobalVarFileChange('global.txt', 4, str(self.x_delta))
                GlobalVarFileChange('global.txt', 5, str(self.y_delta))
                Line(points=[self.x_initial, self.y_initial, 900, touch.y])
...
def GlobalVarFileChange(filename, line, content):
