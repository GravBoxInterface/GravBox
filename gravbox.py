...
os.chdir(r'C:\Users\Erin\Desktop')
...
class InteractionScreen(Screen): #This is the main screen for drawing and user input. Next steps include nesting layouts and adding/binding buttons
	...
	global drawUtility
        ...
        '''orbitButton = MyButton(text='Test Orbit')
        orbitButton.pos_hint = {'x': 0, 'y':0}
        orbitButton.size_hint = (.15, .2)
        orbitButton.background_color = [.4, .4, .4, 1]
        orbitButton.bind(on_release = drawUtility.orbit)
        self.rightlayout.add_widget(orbitButton)'''
...
class DrawingApp(Widget):
    ...
    '''def orbit(self, *args):
        print('called the orbit function')
        init = [self.x_initial, self.y_initial]
        particle = Particle(opacity = 1, **init)
        x, y = np.loadtxt('test_orbit_circle.csv', delimiter=',', usecols=(2,3), unpack=True)
        for i in np.arange(0, len(x)):
            anim = Animation(x=int(round(x[i])), y=int(round(y[i])), duration = .5)
            anim.start(particle)'''        
